...
(function() {
    ...
    async function handleAnalyze(btn) {
        ...
        if (variants?.length > 0) {
            ...
            let html = `<div style="margin-bottom: 12px; display: none;">
    ...
    async function updateUI(url) {
        ...
        entry.innerHTML = `
            <div style="display: flex; gap: 8px; margin-bottom: 10px;">
                <div style="background: rgba(255,255,255,0.1); padding: 6px 12px; border-radius: 6px; font-size: 11px; display: none;">#${foundUrls.size}</div>
            </div>
            <div style="display: none; color: rgba(255,255,255,0.9); margin-bottom: 12px; font-size: 13px; word-break: break-all; font-family: 'Courier New', monospace;" title="${url}">${truncUrl}</div>
